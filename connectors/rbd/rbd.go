...
func (c *ConnRbd) getRbdHandle() *RBDVolumeIOWrapper {
	conf, err := c.createCephConf()
	if err != nil {
	...
	rbdClient, err := NewRBDClient(c.AuthUserName, poolName, conf, c.ClusterName)
	if err != nil {
		logger.Error("Get rbd client failed", err)
		return nil
	}
	image, err := RBDVolume(rbdClient, c.VolumeID)
	if err != nil {
		logger.Error("Get rbd volume failed", err)
		return nil
	}
	metadata := NewRBDImageMetadata(image, poolName, c.AuthUserName, conf)
	ioWrapper := NewRBDVolumeIOWrapper(metadata)
	return ioWrapper
}
